/* display.c: Routines for printing the Spectrum screen
   ...
   $Id: display.c,v 1.39 2004/05/08 11:50:53 pak21 Exp $
...
/* Si se cambia el atributo de un bloque de 8x8 se ha de redibujar el bloque */
...
void display_line(void)
{
  ...
  /* If we're at the end of the frame, and we've got some data to
	...
	for( i = 0, ptr = inactive_rectangle;
	  ...
	  //ui_error(UI_ERROR_ERROR,"Yep: saliendo display.cpp");
      uidisplay_frame_end();
    }
  }
...
add_rectangle( int y, int x, int w )
{
  ...
  if( ++active_rectangle_count > active_rectangle_allocated ) {
    ...
    ptr =(rectangle*) realloc( active_rectangle, new_alloc * sizeof( struct rectangle ) );
    if( !ptr ) {
...
end_line( int y )
{
  ...
  for( i = 0; i < active_rectangle_count; i++ ) {
    ...
    if( ++inactive_rectangle_count > inactive_rectangle_allocated ) {
      ...
      ptr = (rectangle*)realloc( inactive_rectangle,
	       ...
	       libspectrum_byte ink, libspectrum_byte paper )
{
  ...
  /*if( machine_current->timex ) {
	...
	/*CMGEScreen* pScr=CPsPectrum::GetApp()->GetCurScreen();
	pScr->PutPixel(x,y,rgb_colors[( data & 0x80 ) ? ink : paper]);
	pScr->PutPixel(x+1,y,rgb_colors[( data & 0x40 ) ? ink : paper]);
	pScr->PutPixel(x+2,y,rgb_colors[( data & 0x20 ) ? ink : paper]);
	pScr->PutPixel(x+3,y,rgb_colors[( data & 0x10 ) ? ink : paper]);
	pScr->PutPixel(x+4,y,rgb_colors[( data & 0x08 ) ? ink : paper]);
	pScr->PutPixel(x+5,y,rgb_colors[( data & 0x04 ) ? ink : paper]);
	pScr->PutPixel(x+6,y,rgb_colors[( data & 0x02 ) ? ink : paper]);
	pScr->PutPixel(x+7,y,rgb_colors[( data & 0x01 ) ? ink : paper]);*/
	display_image[y][x+ 0] = ( data & 0x80 ) ? ink : paper;
...
set_border( int x, int y, libspectrum_byte colour )
{
  ...
  /*if( machine_current->timex ) {
    ...
    x <<= 3;
    for( i = 0; i < 8; i++ ) /*pScr->PutPixel(x+i,y,rgb_colors[colour]);*/display_image[y][x+i] = colour;
  //}
